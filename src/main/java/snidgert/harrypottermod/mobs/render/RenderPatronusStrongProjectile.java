...
public class RenderPatronusStrongProjectile extends Render {

	private static final ResourceLocation texture;
	private ModelBase model;

	public RenderPatronusStrongProjectile(final RenderManager renderManager) {
		super(renderManager);
	...
	private ResourceLocation getCustomTexture(final ExpelliarmusProjectile entity) {
		return RenderPatronusStrongProjectile.texture;
	}
	...
	public void doRender(final Entity entity, final double x, final double y, final double z, final float yaw, final float partialTick) {
		GL11.glPushMatrix();
		this.bindTexture(RenderPatronusStrongProjectile.texture);
	...
	static {
		texture = new ResourceLocation("harrypottermod:textures/mobs/PatronusProjectile.png");
	}
}
