...
public class EntityMermaid extends EntityMob implements IMob { //DONE: Refactoring
	...
	public EntityMermaid(final World p_i1695_1_) {
		super(p_i1695_1_);
		...
		this.targetTasks.addTask(3, new EntityAINearestAttackableTarget(this, EntityVillager.class, 0, true, false, null));
	...
	public ItemStack getHeldItem() {
		return EntityMermaid.heldItem;
	}

	static {
		EntityMermaid.heldItem = new ItemStack(MainClass.Trident);
	}
	...
	static class AIMoveRandom extends EntityAIBase {
		...
		private EntityMermaid mermaid;

		public AIMoveRandom(EntityMermaid p_i45859_1_) {
			this.mermaid = p_i45859_1_;
		}
		...
		public void updateTask() {
			...
			int i = this.mermaid.getAge();

			if (i > 100) {
				this.mermaid.func_175568_b(0.0F, 0.0F, 0.0F);
			} else if (this.mermaid.getRNG().nextInt(50) == 0 || !this.mermaid.inWater || !this.mermaid.func_175567_n()) {
				float f = this.mermaid.getRNG().nextFloat() * (float) Math.PI * 2.0F;
				float f1 = MathHelper.cos(f) * 0.2F;
				float f2 = -0.1F + this.mermaid.getRNG().nextFloat() * 0.2F;
				float f3 = MathHelper.sin(f) * 0.2F;
				this.mermaid.func_175568_b(f1, f2, f3);
			}
