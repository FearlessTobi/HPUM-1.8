...
public class BoggartChestOpenClass extends BlockContainer {
	...
	private final String name = "BoggartChestOpen";
	...
	public boolean onBlockActivated(World par1World, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		par1World.setBlockState(pos, MainClass.BoggartChestLocked.getDefaultState());
		if (!par1World.isRemote) {
			final List list1 = par1World.getEntitiesWithinAABB(EntityBoggartDementor.class, new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0, pos.getY() + 1.0, pos.getZ() + 1.0).expand(8.0, 8.0, 8.0));
