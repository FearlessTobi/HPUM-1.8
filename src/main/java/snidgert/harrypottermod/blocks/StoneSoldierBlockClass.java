...
public class StoneSoldierBlockClass extends BlockContainer {
	...
	private final String name = "StoneSoldierBlock";
	...
	public boolean onBlockActivated(World par1World, BlockPos pos, IBlockState state, EntityPlayer par5EntityPlayer, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (par5EntityPlayer.getHeldItem() != null && par5EntityPlayer.getHeldItem().getItem() instanceof WoodWandClass) {
			...
			final EntityStoneSoldier entityboat = new EntityStoneSoldier(par1World, pos.getX(), pos.getY(), pos.getZ());
			if (!par1World.isRemote) {
				par1World.spawnEntityInWorld(entityboat);
				entityboat.setOwnerId(par5EntityPlayer.getUniqueID().toString());
