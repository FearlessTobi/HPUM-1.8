...
public class GoldenEggClass extends BlockContainer {
	...
	private final String name = "GoldenEgg";
	...
	public boolean onBlockActivated(World par1World, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (par1World.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ())).getBlock() == Blocks.water) {
			par1World.playSoundEffect(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, "harrypottermod:Song", 1.0f, 1.2f);
		} else {
			par1World.playSoundEffect(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, "harrypottermod:EggScream", 1.0f, 1.2f);
		}
		par1World.setBlockToAir(pos);
		par1World.setBlockState(pos, MainClass.GoldenEggOpen.getDefaultState());
		return true;
	}
