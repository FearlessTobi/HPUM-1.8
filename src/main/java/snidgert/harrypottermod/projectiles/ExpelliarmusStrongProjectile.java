...
public class ExpelliarmusStrongProjectile extends MagicProjectile {

	public int field_92057_e;

	public ExpelliarmusStrongProjectile(final World par1World) {
		super(par1World);
		this.field_92057_e = 0;
		this.setSize(0.5f, 0.5f);
	}

	public ExpelliarmusStrongProjectile(final World par1World, final double par2, final double par4, final double par6, final double par8, final double par10, final double par12) {
		super(par1World, par2, par4, par6, par8, par10, par12);
		this.field_92057_e = 0;
	}

	public ExpelliarmusStrongProjectile(final World par1World, final EntityLivingBase par2EntityLivingBase, final double par3, final double par5, final double par7) {
		...
		if (!super.worldObj.isRemote) {
			if (par1MovingObjectPosition.entityHit != null && par1MovingObjectPosition.entityHit instanceof EntityLivingBase) {
				if (((EntityLivingBase) par1MovingObjectPosition.entityHit).isPotionActive(MainClass.ShieldEffect)) {
					...
					par1MovingObjectPosition.entityHit.attackEntityFrom(DamageSource.magic, 13.0f);
					if (par1MovingObjectPosition.entityHit == player) {
