...
public class TankWandClass extends Item {
	...
	private final String name = "TankWandClass";

	public TankWandClass() {
	...
	public ItemStack onItemRightClick(final ItemStack itemstack, final World world, final EntityPlayer entityplayer) {
		if (!TankWandClass.Tank) {
			Label_11026: {
				...
				case 10: {
					...
					final int h = movingobjectposition.getBlockPos().getX();
					int j2 = movingobjectposition.getBlockPos().getY();
					final int k2 = movingobjectposition.getBlockPos().getZ();
					if (!world.isRemote) {
						final List list3 = world.getEntitiesWithinAABB(EntityNagini.class, new AxisAlignedBB(h, j2, k2, h + 1.0, j2 + 1.0, k2 + 1.0).expand(2.0, 2.0, 2.0));
					...
					if (world.getBlockState(new BlockPos(h, j2, k2)).getBlock() == Blocks.snow_layer) {
						--j2;
					}
					if (world.isAirBlock(new BlockPos(h, j2 + 1, k2))) {
						world.setBlockState(new BlockPos(h, j2 + 1, k2), Blocks.lava.getDefaultState());
					}
					if (world.isAirBlock(new BlockPos(h + 1, j2 + 1, k2))) {
						world.setBlockState(new BlockPos(h + 1, j2 + 1, k2), Blocks.fire.getDefaultState());
					}
					if (world.isAirBlock(new BlockPos(h - 1, j2 + 1, k2))) {
						world.setBlockState(new BlockPos(h - 1, j2 + 1, k2), Blocks.fire.getDefaultState());
					}
					if (world.isAirBlock(new BlockPos(h, j2 + 1, k2 + 1))) {
						world.setBlockState(new BlockPos(h, j2 + 1, k2 + 1), Blocks.fire.getDefaultState());
					}
					if (world.isAirBlock(new BlockPos(h, j2 + 1, k2 - 1))) {
						world.setBlockState(new BlockPos(h, j2 + 1, k2 - 1), Blocks.fire.getDefaultState());
					}
					final EntityFiendfyreDragon entityboat = new EntityFiendfyreDragon(world, h + 0.5f, j2 + 5.0f, k2 + 0.5f);
				...
				case 13: {
					if (this.getDamage(itemstack) == 13 && !world.isRemote) {
						world.playSoundEffect(((Entity) entityplayer).posX + 0.5, ((Entity) entityplayer).posY + 0.5, ((Entity) entityplayer).posZ + 0.5, "harrypottermod:WandFlick", 1.0f, Item.itemRand.nextFloat() * 0.4f + 0.8f);
				...
				case 14: {
					if (this.getDamage(itemstack) == 14 && !world.isRemote) {
						...
						entityplayer.setInvisible(true);
						((Entity) entityplayer).noClip = true;
						final EntityStagForm entitylevitate2 = new EntityStagForm(world);
				...
				case 15: {
					if (this.getDamage(itemstack) == 15 && !world.isRemote) {
						...
						entityplayer.setInvisible(true);
						((Entity) entityplayer).noClip = true;
						final EntityFalconForm entitylevitate3 = new EntityFalconForm(world);
		...
		return new ItemStack(MainClass.OakStick, 1);
	}
	...
	public void onPlayerStoppedUsing(final ItemStack par1ItemStack, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (this.getDamage(par1ItemStack) == 18 && !par2World.isRemote) {
			final Vec3 look = par3EntityPlayer.getLookVec();
			final LevicorpusStrongProjectile fireball4 = new LevicorpusStrongProjectile(par2World, par3EntityPlayer, 1.0, 1.0, 1.0);
		...
		if (this.getDamage(par1ItemStack) == 19 && !par2World.isRemote) {
			final Vec3 look = par3EntityPlayer.getLookVec();
			final ExpelliarmusStrongProjectile fireball5 = new ExpelliarmusStrongProjectile(par2World, par3EntityPlayer, 1.0, 1.0, 1.0);
		...
		if (this.getDamage(par1ItemStack) == 20 && !par2World.isRemote) {
			final Vec3 look = par3EntityPlayer.getLookVec();
			final PatronusProjectile fireball6 = new PatronusProjectile(par2World, par3EntityPlayer, 1.0, 1.0, 1.0);
		...
		if (this.getDamage(par1ItemStack) == 21 && !par2World.isRemote) {
			final Vec3 look = par3EntityPlayer.getLookVec();
			final ImperioProjectile fireball7 = new ImperioProjectile(par2World, par3EntityPlayer, 1.0, 1.0, 1.0);
		...
		if (this.getDamage(par1ItemStack) == 22 && !par2World.isRemote) {
			final Vec3 look = par3EntityPlayer.getLookVec();
			final CrucioProjectile fireball8 = new CrucioProjectile(par2World, par3EntityPlayer, 1.0, 1.0, 1.0);
		...
		if (this.getDamage(par1ItemStack) == 23 && !par2World.isRemote) {
			final Vec3 look = par3EntityPlayer.getLookVec();
			final AvadakedavraProjectile fireball9 = new AvadakedavraProjectile(par2World, par3EntityPlayer, 1.0, 1.0, 1.0);
